...
if ($VersionInfo.FileVersion -ne '1.6.1.0') { throw "Unexpected FileVersion: $($VersionInfo.FileVersion)" }
if ($VersionInfo.ProductVersion -notlike '1.6.1*') { throw "Unexpected ProductVersion: $($VersionInfo.ProductVersion)" }
...
if ($AssemblyVersion -ne '1.6.1.0') { throw "Unexpected assembly version: $AssemblyVersion" }
...
if ($PublishDirectory) {
  ...
  $PublishedName = 'DevRigorHookActivator-1.6.1.exe'
  ...
  Copy-Item -LiteralPath (Join-Path $Root 'DevRigorHookActivator.cs') `
    -Destination (Join-Path $PublishDirectory 'DevRigorHookActivator-1.6.1.cs') -Force
  ...
  Set-Content -LiteralPath (Join-Path $PublishDirectory 'DevRigorHookActivator-1.6.1.build.json') `
